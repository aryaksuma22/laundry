...
@if (isset($pemesanan))
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 mb-4 pb-4 border-b border-gray-200 dark:border-gray-700">
        ...
        <div>
            ...
            <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                {{ $pemesanan->tanggal_pesan ? $pemesanan->tanggal_pesan->isoFormat('dddd, DD MMM YYYY, HH:mm') : '-' }}
            </dd>
        </div>
        <div>
    ...
    <div
        class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm mb-4 pb-4 border-b border-gray-200 dark:border-gray-700">
        <div>
        ...
        @if ($pemesanan->estimasi_berat)
            <div>
                ...
                <dd class="mt-1 text-gray-900 dark:text-white">{{ number_format($pemesanan->estimasi_berat, 1) }} Kg
                </dd>
            </div>
        @endif
        @if ($pemesanan->tanggal_penjemputan)
            <div>
                ...
                <dd class="mt-1 text-gray-900 dark:text-white">
                    {{ $pemesanan->tanggal_penjemputan->isoFormat('DD MMM YYYY') }},
                    {{ $pemesanan->waktu_penjemputan }}</dd>
            </div>
        @endif
        @if ($pemesanan->instruksi_alamat)
            <div class="sm:col-span-2">
                <dt class="font-medium text-gray-500 dark:text-gray-400">Instruksi Alamat</dt>
                <dd class="mt-1 text-gray-900 dark:text-white whitespace-pre-wrap">{{ $pemesanan->instruksi_alamat }}
                </dd>
            </div>
        @endif
        ...
        @if ($pemesanan->catatan_pelanggan)
            <div class="sm:col-span-2">
                ...
                <dd class="mt-1 text-gray-900 dark:text-white whitespace-pre-wrap">{{ $pemesanan->catatan_pelanggan }}
                </dd>
            </div>
        @endif
    </div>
    ...
    <div
        ...
        <div>
            ...
            <dd class="mt-1">
                <span
                    class="px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full {{ $pemesanan->status_badge_class }}">
        ...
        <div>
            ...
            <dd class="mt-1">
                ...
                @endphp
                <span
                    class="px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full {{ $statusClassBayar }}">
                    {{ $statusBayar }}
                </span>
            </dd>
        </div>
        ...
        <div>
            ...
            <dd class="mt-1 text-gray-900 dark:text-white font-bold">Rp
                {{ number_format($pemesanan->total_harga ?? 0, 0, ',', '.') }}</dd>
        </div>
        ...
        @if ($pemesanan->tanggal_selesai)
            <div>
                ...
                <dd class="mt-1 text-gray-900 dark:text-white">
                    {{ $pemesanan->tanggal_selesai->isoFormat('dddd, DD MMM YYYY, HH:mm') }}</dd>
            </div>
        @endif
    </div>
    ...
    @if ($pemesanan->transaksi)
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm mb-4">
            ...
            <div>
                ...
                <dd class="mt-1 text-gray-900 dark:text-white font-semibold">Rp
                    {{ number_format($pemesanan->transaksi->jumlah_dibayar ?? 0, 0, ',', '.') }}</dd>
            </div>
            <div>
                <dt class="font-medium text-gray-500 dark:text-gray-400">Metode Bayar</dt>
                <dd class="mt-1 text-gray-900 dark:text-white">{{ $pemesanan->transaksi->metode_pembayaran ?: '-' }}
                </dd>
            </div>
            <div>
                <dt class="font-medium text-gray-500 dark:text-gray-400">Tgl Bayar</dt>
                <dd class="mt-1 text-gray-900 dark:text-white">
                    {{ $pemesanan->transaksi->tanggal_pembayaran ? $pemesanan->transaksi->tanggal_pembayaran->isoFormat('dddd, DD MMM YYYY, HH:mm') : '-' }}
                </dd>
            </div>
            @if ($pemesanan->transaksi->catatan_transaksi)
                <div class="sm:col-span-2">
                    ...
                    <dd class="mt-1 text-gray-900 dark:text-white whitespace-pre-wrap">
                        {{ $pemesanan->transaksi->catatan_transaksi }}</dd>
                </div>
            @endif
        </div>
    @endif
    ...
    <div class="mt-6 flex justify-end space-x-3">
        ...
        <button type="button" id="dynamicCloseQuickViewModalButton"
