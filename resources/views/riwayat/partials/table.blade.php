<form id="deleteFormRiwayat" action="{{ route('riwayat.destroy', ['riwayat' => 0]) }}" method="POST">
    @csrf
    @method('DELETE')
        ...
        <tbody>
            @foreach ($riwayats as $riwayat)
                <tr class="border hover:bg-sky-50">
                    <td class="px-4 py-3">
                        <input type="checkbox" name="riwayats[]" value="{{ $riwayat->id }}"
                            class="form-checkbox rounded checkbox-row cursor-pointer" />
                    </td>
                    <td class="px-4 py-3">{{ $riwayat->nama_pelanggan }}</td>

                    <td class="px-4 py-3">{{ $riwayat->tanggal_selesai }}</td>

                    <td class="px-4 py-3">{{ $riwayat->status_pesanan }}</td>
                    <td class="px-4 py-3">{{ $riwayat->alamat }}</td>
                    <td class="px-4 py-3">{{ $riwayat->kontak }}</td>
                    <td class="px-4 py-3">
                        <div class="flex flex-row">
                            ...
                            <!-- Delete -->
                            <a href="#" class="delete-riwayat ml-4" data-id="{{ $riwayat->id }}">
